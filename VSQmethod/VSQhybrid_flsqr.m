function [Sigma, info] = VSQhybrid_flsqr(H, volt, varargin)
%VSQhybrid_flsqr Hybrid version of FLSQR algorithm [...]
%
% options  = VSQhybrid_flsqr('defaults')
% [sigma,info] = VSQhybrid_flsqr(H,volt,K,options)
%
%   
% Inputs:
...
%                   [  non-neg. scalar | {'wgcv'} | 'gcv' | 'optimal']
...
% Note: the options structure can be created using the function VSQset. 
%
% Outputs:
%   X : computed solutions, stored column-wise (at the iterations listed in K)
%   info: structure with the following fields:
%      its      - number of the last computed iteration
%      saved_iterations - iteration numbers of iterates stored in Sigma 
...
elseif nargin == 1 
    ...
    if nargout <= 1 && isequal(H,'defaults')
        Sigma = defaultopt;
        return;
    else
        error('Not enough input arguments')
...
options = VSQset(defaultopt, options);

MaxIter    = VSQget(options, 'MaxIter',    [], 'fast');
RegParam   = VSQget(options, 'RegParam',   [], 'fast');
x_true     = VSQget(options, 'x_true',     [], 'fast');
NoStop     = VSQget(options, 'NoStop',     [], 'fast');
IterBar    = VSQget(options, 'IterBar',    [], 'fast');
omega      = VSQget(options, 'GCVweight',  [], 'fast');
stopGCV    = VSQget(options, 'stopGCV',    [], 'fast');
resdegflat = VSQget(options, 'resflatTol', [], 'fast');
degflat    = VSQget(options, 'GCVflatTol', [], 'fast');
regPflat   = VSQget(options, 'regPflatTol', [], 'fast');
mintol     = VSQget(options, 'GCVminTol',  [], 'fast');
NoiseLevel = VSQget(options, 'NoiseLevel', [], 'fast');
eta        = VSQget(options, 'eta',        [], 'fast');
RegParamk  = VSQget(options, 'RegParam0',  [], 'fast');
restart    = VSQget(options, 'restart',    [], 'fast');
verbose    = VSQget(options, 'verbosity',  [], 'fast');
DecompOut  = VSQget(options, 'DecompOut', [], 'fast');
tolX       = VSQget(options, 'tolX',      [], 'fast');
q          = VSQget(options, 'qnorm', [], 'fast');
hybridvariant = VSQget(options, 'hybridvariant', [], 'fast');
...
% note that there is no control on K, as it does not go through VSQset
...
if restart
    ktotcount  = VSQget(options, 'ktotcount', [],'fast');
    TotIterMax = VSQget(options, 'TotIterMax',[],'fast');
    ...
    Ktot = VSQget(options, 'Ktot', [], 'fast');
    % no checks on Ktot, it should be given from IRrestart
end

d = Atransp_times_vec(H, volt);
n = length(d);
m = length(volt);
